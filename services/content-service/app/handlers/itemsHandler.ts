...
function extractPathParameters(path: string): Record<string, string> {
  ...
  const cleanPath = path.replace(/^\/items/, "");
...
export const handler = async (
  ...
  try {
    console.log(
      "Items handler - Raw incoming event:",
    ...
    const routePath = (event.path || "").replace(/^\/items/, "") || "";

    console.log("Items routing:", { httpMethod, routePath, pathParameters });

    let result: APIGatewayProxyResult;

    if (httpMethod === "GET" && routePath === "") {
      console.log("Route: Get all items");
