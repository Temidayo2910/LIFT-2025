...
function extractPathParameters(path: string): Record<string, string> {
  ...
  const cleanPath = path.replace(/^\/faqs/, "");
...
export const handler = async (
  ...
  try {
    console.log(
      "FAQs handler - Raw incoming event:",
    ...
    const routePath = (event.path || "").replace(/^\/faqs/, "") || "";

    console.log("FAQs routing:", { httpMethod, routePath, pathParameters });

    let result: APIGatewayProxyResult;

    if (httpMethod === "GET" && routePath === "") {
      console.log("Route: Get all FAQs");
      result = await controller.getAllFAQsForAdmin(event);
